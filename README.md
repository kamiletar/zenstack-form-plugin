...
Плагин является частью монорепозитория и не требует отдельной установки.
...
| -------------------- | -------------- | -------------------------------------------------------------------------- |
| `defaultLocale` (ru) | **Перезапись** | Полностью перезаписывается из схемы                                        |
| Другие локали (en)   | **Merge**      | Сохраняет существующие переводы, добавляет новые ключи, удаляет устаревшие |
